      ...
      {data.map((post: any, index: number) => {
        return (
          <div className='group relative border-[1px] rounded-2xl overflow-hidden cursor-pointer' key={index}>
            <Button
              variant={savedPosts.includes(String(post.id)) ? 'secondary' : 'default'}
              size={'icon'}
              className={cn(
                'hidden absolute top-3 right-3 z-10 group-hover:flex',
                savedPosts.includes(String(post.id)) ? '' : 'bg-red-500'
              )}
