...
const FollowerDialog = ({ user }) => {
    const [searchQuery, setSearchQuery] = useState("");
    return (_jsxs(Dialog, { children: [_jsx(DialogTrigger, { asChild: true, children: _jsxs(Button, { variant: "link", className: "rounded-full px-0", children: [user.followers, " followers"] }) }), _jsxs(DialogContent, { className: "sm:max-w-[425px] lg:max-w-screen-sm", children: [_jsxs(DialogHeader, { children: [_jsxs(DialogTitle, { className: "mb-3 flex items-center", children: [capitalizeFirstLetter(user.first_name), "'s followers "] }), _jsxs("div", { className: "relative w-full mr-auto", children: [_jsx(Input, { placeholder: "Search groups...", className: "pr-10 ", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value) }), _jsx(Search, { className: "absolute top-1/2 right-3 -translate-y-1/2 text-gray-600 w-5" })] })] }), _jsx("div", { className: "min-h-[400px] max-h-[400px] overflow-auto pr-2 ", children: _jsx(FollowerContent, { user: user, searchQuery: searchQuery }) })] })] }));
};
const FollowerContent = ({ user, searchQuery }) => {
    ...
    if (followers.length == 0 && !isLoading) {
        return (_jsxs("div", { className: "h-full w-full flex flex-col gap-2 justify-center items-center", children: [_jsx(SearchX, { className: "w-10 h-10 text-gray-400" }), _jsx("h1", { children: "No follower found." })] }));
    }
    if (followers.length == 0 && isLoading) {
    ...
    return (followers && (_jsx("div", { className: "space-y-1", children: followers.map((user, index) => {
            return (_jsx(DialogTrigger, { asChild: true, children: _jsxs(Button, { className: "flex w-full justify-start gap-5 py-7", variant: "outline", onClick: () => navigate(`/user/${user.id}`), children: [_jsxs(Avatar, { className: "", children: [_jsx(AvatarImage, { src: user.pf_img_url, className: "object-cover w-full h-full" }), _jsx(AvatarFallback, { children: "CN" })] }), _jsxs("div", { className: "flex gap-2 items-center", children: [_jsxs("div", { className: "flex flex-col justify-start items-start", children: [_jsx("h1", { className: "text-md font-semibold", children: user.first_name + " " + user.last_name }), _jsx("p", { className: "text-xs text-muted-foreground", children: user.email })] }), user.is_following ? (_jsxs("div", { className: "flex items-center text-gray-400", children: [_jsx(Dot, {}), _jsx("h1", { className: "text-xs font-normal", children: "followed" })] })) : ("")] })] }, user.id) }, index));
        }) })));
};
