...
import { ArrowDownAZ, ArrowUpAZ, Ban, Dot, FilterX, Globe, Lock, Pin, PinOff, Search, Users } from 'lucide-react';
      ...
      <DialogTrigger asChild>
        <Button
          ...
          className={cn(
            ...
            savedPosts.includes(String(postId))
              ? 'bg-red-500 text-secondary bg-opacity-70 hover:bg-opacity-100 hover:bg-red-500'
              : ''
          )}
        >
          {savedPosts.includes(String(postId)) ? <PinOff className='w-5 h-5' /> : <Pin className='w-5 h-5' />}
        </Button>
      </DialogTrigger>
