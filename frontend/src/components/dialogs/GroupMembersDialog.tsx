    ...
    <Dialog>
      <DialogTrigger asChild>
        {type == "link" ? (
          ...
          <p className="text-sm w-full px-2 py-1.5 hover:bg-secondary rounded-sm">Members</p>
        )}
      </DialogTrigger>
              ...
              <div key={index} className="flex w-full justify-between px-2 py-2 rounded-md border-[1px]">
                <div className="flex w-full gap-5">
                  ...
                  <div className="flex gap-2 items-center">
                    <h1 className="text-md hover:underline hover:text-primary cursor-pointer" onClick={() => navigate(`/user?id=${user.id}`)}>
                      {user.username}
                    </h1>
